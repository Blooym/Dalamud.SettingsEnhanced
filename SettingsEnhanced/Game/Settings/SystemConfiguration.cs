...
#pragma warning restore CS8618
        ...
        public void ApplyToGame()
        {
            foreach (var prop in typeof(SystemConfiguration).GetProperties(Plugin.ConfigReflectionBindingFlags))
            {
                ...
                {
                    if (GameConfigUtil.TryGetGameConfigValue(configOptionAttribute.ConfigOption, prop.PropertyType, out var refreshedValue))
                    {
                ...
                if (prop.GetValue(this) is object value)
                {
                    GameConfigUtil.SetGameConfigValue(
                        configOptionAttribute.ConfigOption,
                        prop.PropertyType,
                        value
                    );
        ...
        private sealed class JsonConverter : JsonConverter<SystemConfiguration>
        {
            ...
            public override void WriteJson(JsonWriter writer, SystemConfiguration? value, JsonSerializer serializer)
            {
                ...
                foreach (var prop in typeof(SystemConfiguration).GetProperties(Plugin.ConfigReflectionBindingFlags))
                {
                    ...
                    {
                        Plugin.Log.Verbose($"Serialising persisted property {prop.Name}:{currentValue} ({prop.PropertyType}) on {typeof(SystemConfiguration)}");
            ...
            public override SystemConfiguration ReadJson(JsonReader reader, Type objectType, SystemConfiguration? existingValue, bool hasExistingValue, JsonSerializer serializer)
            {
                ...
                foreach (var jproperty in jObject.Properties())
                {
                    ...
                    if (property is not null)
                    {
                        ...
                        Plugin.Log.Verbose($"Deserialising persisted property {property.Name}:{value} ({property.PropertyType}) on {typeof(SystemConfiguration)}");
                    }
                }
