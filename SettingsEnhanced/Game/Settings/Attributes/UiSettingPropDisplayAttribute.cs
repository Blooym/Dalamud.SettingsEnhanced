...
namespace SettingsEnhanced.Game.Settings.Attributes
{
    ...
    internal sealed class UiSettingPropDisplayAttribute(uint nameAddonRow, uint groupAddonRow, uint headerAddonRow, bool interfaceIndented = false) : Attribute
    {
        ...
        public string UiGroup { get; } = groupAddonRow == 0 ? string.Empty : Plugin.AddonSheet.GetRow(groupAddonRow).Text.ExtractText();
        public string UiHeader { get; } = headerAddonRow == 0 ? string.Empty : Plugin.AddonSheet.GetRow(headerAddonRow).Text.ExtractText();
